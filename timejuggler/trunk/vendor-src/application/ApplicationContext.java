...
/**
 * A singleton that manages shared objects, like actions, resources, and tasks, for {@code Applications}.
 * <p/>
 * {@link Application Applications} use the {@code ApplicationContext} singleton to find global values and services.
 * The majority of the Swing Application Framework API can be accessed through {@code ApplicationContext}.  The static
 * {@code getInstance} method returns the singleton Typically it's only called after the application has been {@link
 * Application#launch launched}, however it is always safe to call {@code getInstance}.
...
public class ApplicationContext extends AbstractBean {
    ...
    /**
     * ApplicationContext is a singleton that's lazily constructed when this method is called.  Typically it's
     * constructed as a side effect of calling {@link Application#launch Application.launch()}.
    ...
    /**
     * ApplicationContext is a singleton that's lazily constructed by {@code getInstance()}.
     * @see #getInstance
     */
    ...
    /**
     * Returns the application's class or null if the application hasn't been launched and this property hasn't been
     * set.  Once the application has been launched, the value returned by this method is the same as {@code
     ...
     * <p/>
     * This method is only intended for testing, or design time configuration.  Normal applications shouldn't need to
     * call it directly.
    ...
    /**
     * The application's {@code ResourceManager} provides read-only cached access to resources in ResourceBundles via
     * the {@link ResourceMap ResourceMap} class.
    ...
    /**
     * Change this application's {@code ResourceManager}.  An {@code ApplicationContext} subclass that wanted to
     * fundamentally change the way {@code ResourceMaps} were created and cached could replace this property in its
     * constructor.
     * <p/>
     ...
     * encapsulates the {@code ResourceBundles} for the specified class.
     * <p/>
     * This is just a convenience method that calls {@link ResourceManager#getResourceMap(Class)
    ...
    /**
     * Returns the {@link ResourceMap#getParent chain} of ResourceMaps that's shared by the entire application,
     * beginning with the one defined for the Application class, i.e. the value of the {@code applicationClass}
     * property.
     * <p/>
    ...
    /**
     * Change this application's {@code ActionManager}.  An {@code ApplicationContext} subclass that wanted to
     * fundamentally change the way {@code ActionManagers} were created and cached could replace this property in its
     * constructor.
     * <p/>
     ...
     * <p/>
     * This is just a convenience method that calls {@link ActionManager#getActionMap() ActionManager.getActionMap()}.
     * It's defined as:
     * <pre>
     ...
     * <p/>
     * This is just a convenience method that calls {@link ActionManager#getActionMap()
    ...
    /**
     * Returns the default TaskService, i.e. the one named "default": <code>return getTaskService("default")</code>.
     * The {@link ApplicationAction#actionPerformed ApplicationAction actionPerformed} method executes background
     * <code>Tasks</code> on the default TaskService.  Application's can launch Tasks in the same way, e.g.
     * <pre>
    ...
    /**
     * Returns a shared TaskMonitor object.  Most applications only need one TaskMonitor for the sake of status bars and
