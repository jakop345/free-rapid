 ...
 * <p/>
 * This class takes care of component property injection, exit processing, and saving/restoring session state in a way
 * that's appropriate for simple single-frame applications.  The application's JFrame is created automatically, with a
 * WindowListener that calls exit() when the window is closed.  Session state is stored when the application shuts down,
 * and restored when the GUI is shown.
 * <p/>
 * To use {@code SingleFrameApplication}, one need only override {@code startup}, create the GUI's main panel, and apply
 ...
 * }
 * </pre>
 * The call to {@code show} in this example creates a JFrame (named "mainFrame"), that contains the "Hello World"
 * JLabel.  Before the frame is made visible, the properties of all of the components in the hierarchy are initialized
 * with {@link ResourceMap#injectComponents ResourceMap.injectComponents} and then restored from saved session state (if
 * any) with {@link SessionStorage#restore SessionStorage.restore}. When the application shuts down, session state is
 * saved.
 * <p/>
 * A more realistic tiny example would rely on a ResourceBundle for the JLabel's string and the main frame's title.  The
 * automatic injection step only initializes the properties of named components, so:
 * <pre>
 * class MyApplication extends SingleFrameApplication {
 *     &#064;Override public void startup(String[] args) {
 ...
 * }
 * </pre>
 * The ResourceBundle should contain definitions for all of the standard Application resources, as well the main frame's
 * title and the label's text.  Note that the JFrame that's implicitly created by the {@code show} method  is named
 * "mainFrame".
 * <pre>
...
public abstract class SingleFrameApplication extends Application {
     ...
     * <p/>
     * The frame's name is set to "mainFrame", its title is initialized with the value of the {@code Application.title}
     * resource and a {@code WindowListener} is added that calls {@code exit} when the user attempts to close the
     * frame.
     * <p/>
     * <p/>
     ...
     * <p/>
     * This method should be called from the startup method by a subclass that wants to construct and initialize the
     * main frame itself.  Most applications can rely on the fact that {code getMainFrame} lazily constructs the main
     * frame and initializes the {@code mainFrame} property.
     * <p/>
     * If the main frame property was already initialized, either implicitly through a call to {@code getMainFrame} or
     * by explicitly calling this method, an IllegalStateException is thrown.  If {@code mainFrame} is null, an
     ...
     * <p/>
     * By default the {@code show} methods {@link ResourceMap#injectComponents inject resources} before initializing the
     * JFrame or JDialog's size, location, and restoring the window's session state.  If the app is showing a window
     * whose resources have already been injected, or that shouldn't be initialized via resource injection, this method
     * can be overridden to defeat the default behavior.
    ...
    /**
     * Show the specified component in the {@link #getMainFrame main frame}. Typical applications will call this method
     * after constructing their main GUI panel in the {@code startup} method.
     * <p/>
     * Before the main frame is made visible, the properties of all of the components in the hierarchy are initialized
     * with {@link ResourceMap#injectComponents ResourceMap.injectComponents} and then restored from saved session state
     * (if any) with {@link SessionStorage#restore SessionStorage.restore}.  When the application shuts down, session
     * state is saved.
     * <p/>
     * Note that the name of the lazily created main frame (see {@link #getMainFrame getMainFrame}) is set by default.
     * Session state is only saved for top level windows with a valid name and then only for component descendants that
     * are named.
     * <p/>
     ...
     * <p/>
     * This method is intended for showing "secondary" windows, like message dialogs, about boxes, and so on.  Unlike
     * the {@code mainFrame}, dismissing a secondary window will not exit the application.
     * <p/>
     * Session state is only automatically saved if the specified JDialog has a name, and then only for component
     * descendants that are named.
     * <p/>
     * Throws an IllegalArgumentException if {@code c} is null
     * @param c the main frame's contentPane child
     * @see #show(JComponent)
     ...
     * the {@code mainFrame}, dismissing a secondary window will not exit the application.
     * <p/>
     * Session state is only automatically saved if the specified JFrame has a name, and then only for component
     * descendants that are named.
     * <p/>
     * Throws an IllegalArgumentException if {@code c} is null
     * @see #show(JComponent)
    ...
    /**
     * Return all of the visible JWindows, JDialogs, and JFrames per Window.getWindows() on Java SE 6, or
     * Frame.getFrames() for earlier Java versions.
     */
