...
/**
 * This class is intended to serve as the model for GUI components, like status bars, that display the state of an
 * application's background tasks.  {@code TaskMonitor} provides an overview of all the ApplicationContext's Tasks, as
 * well as the state of a single {@code foreground} Task.
 * <p/>
 * <p/>
 * The value of {@link #getTasks getTasks()} is a list of all of the {@code Tasks} whose state is not {@link Task#isDone
 * DONE} for all of the ApplicationContext's {@code TaskServices}.  In other words: all of the ApplicationContext's
 * background tasks that haven't finished executing.  Each time a new TaskService Task is executed it's added to the
 * list; when the Task finishes it's removed.  Each time the list changes {@code PropertyChangeListeners} are fired.
 * Applications that wish to create a detailed visualization of all Tasks should monitor the TaskMonitor {@code "tasks"}
 * property.
 * <p/>
 * <p/>
 * Users are often only interested in the status of a single <i>foreground</i> task, typically the one associated with
 * GUI element they're working with, or with the most recent command they've issued.  The TaskMonitor's
 * PropertyChangeListener is notified each time a property of the {@link #setForegroundTask foregroundTask} changes.
 * Additionally the TaskMonitor fires synthetic PropertyChangeEvents for properties named "pending", "started", and
 * "done" when the corresponding Task {@code state} property changes occur.
 * <p/>
 * <p/>
 * TaskMonitor manages a queue of new Tasks.  The foregroundTask is automatically set to the first new Task, and when
 * that Task finishes, the next Task in the queue, and so on. Applications can set the foregroundTask explicitly, to
 * better reflect what the user is doing.  For example, a tabbed browsing GUI that launched one Task per tab might set
 * the foreground Task each time the user selected a tab.  To prevent the foregroundTask property from (ever) being
 * reset automatically, one must set {@link #setAutoUpdateForegroundTask autoUpdateForegroundTask} to false.
 * <p/>
 * <p/>
 * This class is not thread-safe.  All of its methods must be called on the event dispatching thread (EDT) and all of
 * its listeners will run on the EDT.
...
public class TaskMonitor extends AbstractBean {
    ...
    /**
     * The TaskMonitor's PropertyChangeListeners are fired each time any property of the the {@code foregroundTask}
     * changes.  By default this property is set to the first Task to be executed and then, when that Task finishes,
     * reset to the next most recently executed Task.  If the {@code autoUpdateForegroundTask} is false, then the
     * foregroundTask property is not reset automatically.
    ...
    /**
     * Indicates the {@code Task} whose status the ApplicationContext's GUI wants to be displayed, typically in the main
     * window's status bar.
    ...
    /**
     * True if the {@code foregroundTask} property should be automatically reset to the oldest Task in the queue when it
     * finishes running.
     * <p/>
    ...
    /**
     * True if the {@code foregroundTask} property should be automatically reset to the oldest Task in the queue when it
     * finishes running.  An application that wants explicit control over the Task being monitored can set this property
     * to false.
     * <p/>
     * This property is true by default.
     ...
     * <p/>
     * Each time the list of Tasks changes, a PropertyChangeEvent for the property named "tasks" is fired.  Applications
     * that want to monitor all background Tasks should monitor the tasks property.
