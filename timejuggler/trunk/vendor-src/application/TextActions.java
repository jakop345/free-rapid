 ...
 * <p/>
 * This class only exists to paper over limitations in the standard JTextComponent cut/copy/paste/delete
 * javax.swing.Actions.  The standard cut/copy Actions don't keep their enabled property in sync with having the focus
 * and (for copy) having a non-empty text selection.  The standard paste Action's enabled property doesn't stay in sync
 * with the current contents of the clipboard.  The paste/copy/delete actions must also track the JTextComponent
 * editable property.
 * <p/>
 * The new cut/copy/paste/delete are installed lazily, when a JTextComponent gets the focus, and before any other
 * focus-change related work is done.  See updateFocusOwner().
