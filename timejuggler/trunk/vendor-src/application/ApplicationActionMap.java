...
/**
 * An {@link javax.swing.ActionMap ActionMap} class where each entry corresponds to an <tt>&#064;Action</tt> method from
 * a single <tt>actionsClass</tt> (i.e. a class that contains one or more <tt>&#064;Actions</tt>).  Each entry's key is
 * the <tt>&#064;Action's</tt> name (the method name by default), and the value is an {@link ApplicationAction} that
 * calls the <tt>&#064;Actions</tt> method. For example, the code below prints <tt>"Hello World"</tt>:
 * <pre>
 ...
 * <p/>
 * <p/>
 * If a <tt>ResourceMap</tt> is provided then each <tt>ApplicationAction's</tt> ({@link javax.swing.Action#putValue
 * putValue}, {@link javax.swing.Action#getValue getValue}) properties are initialized from the ResourceMap.
 * <p/>
 * <p/>
...
public class ApplicationActionMap extends ActionMap {
    ...
    /**
     * All of the {@code @ProxyActions} recursively defined by this {@code ApplicationActionMap} and its parent
     * ancestors.
     * <p/>
     * Returns a read-only list of the {@code @ProxyActions} defined by this {@code ApplicationActionMap's} {@code
     * actionClass} and, recursively, by this {@code ApplicationActionMap's} parent. If there are no proxyActions, an
