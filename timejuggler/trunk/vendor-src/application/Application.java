 ...
 * <p/>
 * <p/>
 * This class defines defines a simple lifecyle for Swing applications: {@code startup}, {@code ready}, and {@code
 * shutdown}. The {@code Application's} {@code startup} method is responsible for creating the initial GUI and making it
 * visible, and the {@code shutdown} method for hiding the GUI and performing any other cleanup actions before the
 * application exits.  The {@code ready} method is for applications that want to do a little bit of extra work once the
 * GUI is "ready" to use.  Concrete subclasses must override the {@code startup} method.
 * <p/>
 * Applications are started with the static {@code launch} method. Applications use the {@code ApplicationContext}
 * {@link ApplicationContext#getInstance singleton} to find resources, actions, local storage, and so on.
 * <p/>
...
public abstract class Application extends AbstractBean {
    ...
    private class DoWaitForEmptyEventQ extends Task<Void, Void> {
        DoWaitForEmptyEventQ() {
            setTitle("DoWaitForEmptyEventQ");
            setDescription("Wait for the event dispatching thread's queue to become empty");
        }
