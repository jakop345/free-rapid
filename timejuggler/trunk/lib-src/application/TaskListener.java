...
public interface TaskListener<T, V> {
    ...
    /**
     * Called just before the Task's {@code doInBackground} method is called, 
     * i.e. just before the task begins running.
     * 
    ...
    /**
     * Called each time the Task's {@code process} method is called.
     * The {@link TaskEvent#getValue value} of the event is the list
     * of values passed to the process method.  
     * 
    ...
    /**
     * Called after the Tasks done() method is called.  The event's value 
     * is the value returned by the Task's {@link Task#get get} method,
     * i.e. the value that's computed by the Task's 
     * {@link Task#doInBackground doInBackground} method.
     *
