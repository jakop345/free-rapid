 ...
 * MyClass}, and the rest of the chain contains resources shared by the entire application.
 * <p/>
 * Resources for a class are defined by an eponymous {@code ResourceBundle} in a {@code resources} subpackage.  The
 * Application class itself may also provide resources. A complete description of the naming conventions for
 * ResourceBundles is provided by the {@link #getResourceMap(Class) getResourceMap()} method.
 * <p/>
 * The mapping from classes and {@code Application} to a list ResourceBundle names is handled by two protected methods:
 * {@link #getClassBundleNames(Class) getClassBundleNames}, {@link #getApplicationBundleNames()
 * getApplicationBundleNames}. Subclasses could override these methods to append additional ResourceBundle names to the
 * default lists.
...
public class ResourceManager extends AbstractBean {
    ...
    /**
     * Returns a {@link ResourceMap#getParent chain} of {@code ResourceMaps} that encapsulate the {@code
     * ResourceBundles} for each class from {@code startClass} to (including) {@code stopClass}.  The final link in the
     * chain is Application ResourceMap chain, i.e. the value of {@link #getResourceMap() getResourceMap()}.
     * <p/>
     * The ResourceBundle names for the chain of ResourceMaps are defined by  {@link #getClassBundleNames} and {@link
     * #getApplicationBundleNames}.  Collectively they define the standard location for {@code ResourceBundles} for a
     * particular class as the {@code resources} subpackage.  For example, the ResourceBundle for the single class
     * {@code com.myco.MyScreen}, would be named {@code com.myco.resources.MyScreen}.  Typical ResourceBundles are
     * ".properties" files, so: {@code com/foo/bar/resources/MyScreen.properties}.  The following table is a list of the
     * ResourceMaps and their constituent ResourceBundles for the same example:
     * <p/>
     ...
     * <td>com/myco/resources/MyScreen.properties</td> </tr> <tr> <td>2/td> <td>application: com.myco.MyApp</td>
     * <td>com.myco.resources.MyApp</td> <td>com/myco/resources/MyApp.properties</td> </tr> <tr> <td>3</td>
     * <td>application: javax.swing.application.Application</td> <td>javax.swing.application.resources.Application</td>
     ...
     * same name then the one earlier in the list has precedence
     * <p/>
     * ResourceMaps are constructed lazily and cached.  One ResourceMap is constructed for each sequence of classes in
     * the same package.
     ...
     * @return a {@code ResourceMap} chain that contains resources loaded from {@code ResourceBundles}  found in the
     *         resources subpackage for each class.
    ...
    /**
     * Return the ResourcedMap chain for the specified class. This is just a convenince method, it's the same as:
    ...
    /**
     * Returns the chain of ResourceMaps that's shared by the entire application, beginning with the resources defined
     ...
     * property is provided by the {@link #getApplicationBundleNames} method.
     * @see #setApplicationBundleNames
     */
     ...
     * <p/>
     * This method is used by the {@code getResourceMap} methods to compute the list of ResourceBundle names for a new
     * {@code ResourceMap}.  ResourceManager subclasses can override this method to add additional class-specific
    ...
    /**
     * Called by {@link #getResourceMap} to construct {@code ResourceMaps}. By default this method is effectively just:
     * <pre>
    ...
    /**
     * The value of the special Application ResourceMap resource named "platform".  By default the value of this
     * resource is "osx" if the underlying operating environment is Apple OSX or "default".
    ...
    /**
     * Defines the value of the special Application ResourceMap resource named "platform".  This resource can be used to
     * define platform specific resources.  For example:
     * <pre>
     ...
     * </pre>
     * <p/>
     * By default the value of this resource is "osx" if the underlying operating environment is Apple OSX or "default".
     * To distinguish other platforms one can reset this property based on the value of the {@code "os.name"} system
     * property.
     * <p/>
     * This method should be called as early as possible, typically in the Application {@link Application#initialize
     * initialize} method.
     * @see #getPlatform
