...
/**
 * The {@link javax.swing.Action} class used to implement the <tt>&#064;Action</tt> annotation.  This class is typically
 * not instantiated directly, it's created as a side effect of constructing an <tt>ApplicationActionMap</tt>:
 * <pre>
 ...
 * <p/>
 * <p/>
 * When an ApplicationAction is constructed, it initializes all of its properties from the specified
 * <tt>ResourceMap</tt>.  Resource names must match the {@code @Action's} name, which is the name of the corresponding
 * method, or the value of the optional {@code @Action} name parameter.  To initialize the text and shortDescription
 * properties of the action named <tt>"anAction"</tt> in the previous example, one would define two resources:
 * <pre>
 ...
 * {@link #ApplicationAction constructor} documentation.
 * <p/>
 * <p/>
 * An ApplicationAction's <tt>enabled</tt> and <tt>selected</tt> properties can be delegated to boolean properties of
 * the Actions class, by specifying the corresponding property names. This can be done with the {@code @Action}
 * annotation, e.g.:
 * <pre>
 * public class MyActions {
 ...
 * }
 * </pre>
 * If the MyActions class supports PropertyChange events, then then ApplicationAction will track the state of the
 * specified property ("anActionEnabled" in this case) with a PropertyChangeListener.
 * <p/>
 * <p/>
 * ApplicationActions can automatically <tt>block</tt> the GUI while the <tt>actionPerformed</tt> method is running,
 * depending on the value of block annotation parameter.  For example, if the value of block is
 * <tt>Task.BlockingScope.ACTION</tt>, then the action will be disabled while the actionPerformed method runs.
 * <p/>
 * <p/>
 * An ApplicationAction can have a <tt>proxy</tt> Action, i.e. another Action that provides the <tt>actionPerformed</tt>
 * method, the enabled/selected properties, and values for the Action's long and short descriptions.  If the proxy
 * property is set, this ApplicationAction tracks all of the aforementioned properties, and the <tt>actionPerformed</tt>
 * method just calls the proxy's <tt>actionPerformed</tt> method.  If a <tt>proxySource</tt> is specified, then it
 * becomes the source of the ActionEvent that's passed to the proxy <tt>actionPerformed</tt> method.  Proxy action
...
public class ApplicationAction extends AbstractAction {
     ...
     * <p/>
     * <p/>
     * If a {@code ResourceMap} is provided, then all of the {@link javax.swing.Action Action} properties are
     * initialized with the values of resources whose key begins with {@code baseName}. ResourceMap keys are created by
     * appending an &#064;Action resource name, like "Action.shortDescription" to the &#064;Action's baseName For
     * example, Given an &#064;Action defined like this:
     * <pre>
     * &#064;Action void actionBaseName() { }
     * </pre>
     * <p/>
     * Then the shortDescription resource key would be <code>actionBaseName.Action.shortDescription</code>, as in:
     * <pre>
     ...
     * <p/>
     * <p/>
     * A few the resources are handled specially: <ul> <li><tt>Action.text</tt><br> Used to initialize the Action
     * properties with keys <tt>Action.NAME</tt>, <tt>Action.MNEMONIC_KEY</tt> and <tt>Action.DISPLAYED_MNEMONIC_INDEX</tt>.
     * If the resources's value contains an "&" or an "_" it's assumed to mark the following character as the mnemonic.
     * If Action.mnemonic/Action.displayedMnemonic resources are also defined (an odd case), they'll override the
     * mnemonic specfied with the Action.text marker character.
     * <p/>
     * <li><tt>Action.icon</tt><br> Used to initialize both ACTION.SMALL_ICON,LARGE_ICON.  If Action.smallIcon or
     * Action.largeIcon resources are also defined they'll override the value defined for Action.icon.
     * <p/>
     * <li><tt>Action.displayedMnemonicIndexKey</tt><br> The corresponding javax.swing.Action constant is only defined
     * in Java SE 6. We'll set the Action property in Java SE 5 too. </ul>
    ...
    /**
     * The name of the {@code @Action} enabledProperty whose value is returned by {@link #isEnabled isEnabled}, or
     * null.
    ...
    /**
     * The name of the {@code @Action} selectedProperty whose value is returned by {@link #isSelected isSelected}, or
     * null.
    ...
    /**
     * Set the proxy for this action.  If the proxy is non-null then we delegate/track the following: <ul>
     * <li><tt>actionPerformed</tt><br> Our <tt>actionPerformed</tt> method calls the delegate's after the ActionEvent
     * source to be the value of <tt>getProxySource</tt>
     * <p/>
     * <li><tt>shortDescription</tt><br> If the proxy's shortDescription, i.e. the value for key {@link
     * javax.swing.Action#SHORT_DESCRIPTION SHORT_DESCRIPTION} is not null, then set this action's shortDescription.
     * Most Swing components use the shortDescription to initialize their tooltip.
     * <p/>
     * <li><tt>longDescription</tt><br> If the proxy's longDescription, i.e. the value for key {@link
     * javax.swing.Action#LONG_DESCRIPTION LONG_DESCRIPTION} is not null, then set this action's longDescription. </ul>
     * @see #setProxy
     * @see #setProxySource
     * @see #actionPerformed
     */
    ...
    /**
     * Return the value that becomes the <tt>ActionEvent</tt> source  before the ActionEvent is passed along to the
     * proxy Action.
    ...
    /**
     * Set the value that becomes the <tt>ActionEvent</tt> source before the ActionEvent is passed along to the proxy
     * Action.
    ...
    */
    ...
    /**
     * The name of this Action.  This string begins with the name the corresponding &#064;Action method (unless the
     * <tt>name</tt> &#064;Action parameter was specified).
     * <p/>
     * <p/>
     * This name is used as a prefix to look up action resources, and the ApplicationContext Framework uses it as the
     * key for this Action in ApplicationActionMaps.
     * <p/>
     * <p/>
     * Note: this property should not confused with the {@link javax.swing.Action#NAME Action.NAME} key.  That key is
     * actually used to initialize the <tt>text</tt> properties of Swing components, which is why we call the
     * corresponding ApplicationAction resource "Action.text", as in:
     * <pre>
    ...
    /**
     * Provides parameter values to &#064;Action methods.  By default, parameter values are selected based exclusively
     * on their type: <table border=1> <tr> <th>Parameter Type</th> <th>Parameter Value</th> </tr> <tr>
     * <td><tt>ActionEvent</tt></td> <td><tt>actionEvent</tt></td> </tr> <tr> <td><tt>javax.swing.Action</tt></td>
     * <td>this <tt>ApplicationAction</tt> object</td> </tr> <tr> <td><tt>ActionMap</tt></td> <td>the <tt>ActionMap</tt>
     * that contains this <tt>Action</tt></td> </tr> <tr> <td><tt>ResourceMap</tt></td> <td>the <tt>ResourceMap</tt> of
     * the the <tt>ActionMap</tt> that contains this <tt>Action</tt></td> </tr> <tr>
     * <td><tt>ApplicationContext</tt></td> <td>the value of <tt>ApplicationContext.getInstance()</tt></td> </tr>
     * </table>
     * <p/>
     * <p/>
     * ApplicationAction subclasses may also select values based on the value of the <tt>Action.Parameter</tt>
     * annotation, which is passed along as the <tt>pKey</tt> argument to this method:
     * <pre>
     ...
     * <p/>
     * <p/>
     * If <tt>pType</tt> and <tt>pKey</tt> aren't recognized, this method calls {@link #actionFailed} with an
     ...
     * <p/>
     * If there's a proxy Action then call its actionPerformed method.  Otherwise, call the &#064;Action method with
     * parameter values provided by {@code getActionArgument()}.  If anything goes wrong call {@code actionFailed()}.
    ...
    /**
     * If the proxy action is null and {@code enabledProperty} was specified, then return the value of the enabled
     * property's is/get method applied to our ApplicationActionMap's {@code actionsObject}. Otherwise return the value
    ...
    /**
     * If the proxy action is null and {@code enabledProperty} was specified, then set the value of the enabled property
     * by invoking the corresponding {@code set} method on our ApplicationActionMap's {@code actionsObject}. Otherwise
     * set the value of this Action's enabled property.
    ...
    /**
     * If the proxy action is null and {@code selectedProperty} was specified, then return the value of the selected
     * property's is/get method applied to our ApplicationActionMap's {@code actionsObject}. Otherwise return the value
     * of this Action's enabled property.
    ...
    /**
     * If the proxy action is null and {@code selectedProperty} was specified, then set the value of the selected
     * property by invoking the corresponding {@code set} method on our ApplicationActionMap's {@code actionsObject}.
    ...
    /**
     * Keeps the {@code @Action selectedProperty} in sync when the value of {@code key} is {@code Action.SELECTED_KEY}.
    ...
    /**
     * Returns a string representation of this <tt>ApplicationAction</tt> that should be useful for debugging. If the
     * action is enabled it's name is enclosed by parentheses; if it's selected then a "+" appears after the name.  If
     * the action will appear with a text label, then that's included too. If the action has a proxy, then we append the
     * string for the proxy action.
