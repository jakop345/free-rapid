...
/*PŠif (isInUniqueKey())
		addMenuItem(menu, "Remove from unique key", "mDisconnect.gif", event.getComponent(), "removing", this, cz.felk.cvut.erm.event.interfaces.Item.class);
	else
		if (getOwner() instanceof Entity)
		addMenuItem(menu, "Add to unique key", "mUKey.gif", this, "createUnique", this, Atribute.class);
*/
        if (type != CHEN) {
//		if (((cz.felk.cvut.erm.conceptual.beans.Atribute) getModel()).getPosition()>1)
            {
                addMenuItem(menu, "Move to top", "img/mMoveTop.gif", this, "moveTop");
                addMenuItem(menu, "Move up", "img/mMoveUp.gif", this, "moveUp");
            }
//		if (((cz.felk.cvut.erm.conceptual.beans.Atribute) getModel()).getPosition() < ((Entity) getOwner()).attribs.size())
            {
...
*/
        ...
        if (getNotationType() == CHEN) {
            switch (getLevelOfDetails()) {
                ...
                case (Schema.LOD_FULL):
                    ...
                    if (dataType instanceof cz.felk.cvut.erm.datatype.UserDefinedDataType) {
                        final DataTypeManager dataTypeManager = this.getModel().getSchema().getDataTypeManager();
                        if (dataTypeManager.isInNestedNames(dataType.toString()) ||
                                dataTypeManager.isInObjectNames(dataType.toString()) ||
                                dataTypeManager.isInVarrayNames(dataType.toString())) {
     ...
     */
    public void removeAtribute(cz.felk.cvut.erm.event.RemoveEvent ev) {
