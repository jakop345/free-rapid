...
final public class Feeder {
    ...
    private class InputThread extends StoppableThread {
        ...
        public void run() {
            ...
            try {
                ...
                if (replicaNode.getType().isArbiter()) {
                     ...
                     * the replica's name. We need to pass the updater into
                     * FeederReplicaSyncup so that the replica's local CBVLSN
        ...
        private void processHeartbeatResponse(Message response) {
            ...
            final VLSN reportedVLSN = hbResponse.getTxnEndVLSN();
            ...
            if (reportedVLSN.compareTo(repNode.getCurrentTxnEndVLSN()) >= 0) {
                caughtUp = true;
                adviseMasterTransferProgress();
            }
            ...
            final long reportedVLSNSequence = reportedVLSN.getSequence();
            long statCommitVLSN = (commitVLSN <= reportedVLSNSequence) ?
                commitVLSN : reportedVLSNSequence;
            ...
            if (commitVLSN <= reportedVLSNSequence) {
                ...
                final long vlsnLag = commitVLSN - reportedVLSNSequence;
    ...
    private class OutputThread extends StoppableThread {
        ...
        public void run() {
            ...
            try {
                 ...
                 * restore.
                 */
                LoggerUtils.info(logger, repImpl, lfnfe.getMessage());
            } catch (RuntimeException e) {
                shutdownException = e;
