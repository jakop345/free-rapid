...
public class RepParams {
     ...
     * @deprecated
     *
     * It's retained primarily for compatibility and should not be used by new
     * applications. The new file storage based VLSN retention mechanism
     * {@link #REPLAY_COST_PERCENT} and {@link #REPLAY_FREE_DISK_PERCENT}
     * supercedes this functionality.
     *
     * The minimum number of, most recent, VLSNs that are retained as part of
     * the replication stream. This ensures that any requests to sync in the
     * current vlsn range between: [ VLSNRange.getLast() - MIN_RETAINED_VLSNS
     * .. VLSNRange.getLast ] are guaranteed to succeed as long as there is a
     * syncable VLSN in this range.
     *
     * The default value of 200000 vlsns, roughly corresponds to 10K of records
     * each committed in its own transaction. Assuming an average record size
     * of 1K, this represents ~100MB of retained storage.
     *
     * Note that this value must be set consistently across all the RNs in the
     * replication group.
     */
    ...
    /**
     * @see ReplicationConfig#REPLAY_COST_PERCENT
     */
