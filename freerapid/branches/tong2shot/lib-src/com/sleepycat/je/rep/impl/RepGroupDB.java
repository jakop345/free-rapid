...
public class RepGroupDB {
    ...
    public static RepGroupImpl getGroup(RepImpl rImpl,
        ...
        /* Get secondary nodes from their feeders */
    ...
    public void setMinJEVersion(final JEVersion newMinJEVersion)
        ...
        try {
            ...
            LoggerUtils.info(logger, repImpl,
                             "Proceeding without acks for minimum JE version");
        } finally {
            if (txn != null) {
                txn.abort();
            }
     ...
     *
     * <p>This method should not be called for secondary nodes.
     *
    ...
    void ensureMember(RepNodeImpl ensureNode)
        ...
        if (ensureNode.getType().isSecondary()) {
            throw new IllegalArgumentException(
                "Attempt to call ensureMember on SECONDARY node: " +
                ensureNode);
        }
    ...
    public void removeMember(final RepNodeImpl removeNode,
        ...
        if (removeNode.getType().isSecondary()) {
            throw new IllegalArgumentException(
                "Attempt to call removeMember on a SECONDARY node: " +
                removeNode);
        }
    ...
    public void updateMember(final RepNodeImpl node, final boolean quorumAck)
        ...
        if (node.getType().isSecondary()) {
            throw new IllegalArgumentException(
                "Attempt to call updateMember on a SECONDARY node: " + node);
        }
     ...
     * the node is a secondary node.  Also updates the rep node's transient
     * group information about the global CBVLSN. If it encounters contention,
     * it returns false, and the caller must retry at some later point in time.
     *
    ...
    public boolean updateLocalCBVLSN(final NameIdPair nameIdPair,
        ...
        try {
            ...
            /*
             * No database update for secondary nodes, but set ok to true so
             * that the rep node's group information is updated.
             */
    ...
    private void saveNodeObject(Txn txn,
        ...
        assert !node.getType().isSecondary();
