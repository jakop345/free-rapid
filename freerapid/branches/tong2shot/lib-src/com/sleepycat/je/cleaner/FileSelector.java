...
public class FileSelector {
    ...
    /* Used to prevent repeated probing of the same files. */
    private final LinkedList<Long> excludeFilesForCorrection;
    private final static int MAX_EXCLUDE_FILES_FOR_CORRECTION = 5;

    FileSelector() {
        fileInfoMap = new TreeMap<Long, FileInfo>();
        pendingLNs = new HashMap<Long, LNInfo>();
        pendingDBs = new HashSet<DatabaseId>();
        excludeFilesForCorrection = new LinkedList<Long>();
    }
     ...
     */
    synchronized Long selectFileForCleaning(
        ...
        while ((maxBatchFiles == 0) ||
            ...
            final Long fileNum = calculator.getBestFile(
                fileSummaryMap, forceCleaning, toBeCleaned > 0 /*isBacklog*/,
     ...
     */
    synchronized boolean addPendingLN(
        long logrecLsn,
        LN ln,
        DatabaseId dbId,
        byte[] key) {
        ...
        boolean added = pendingLNs.put(
            Long.valueOf(logrecLsn), new LNInfo(ln, dbId, key)) != null;
     ...
     */
    synchronized NavigableSet<Long> copySafeToDeleteFiles() {
        final NavigableSet<Long> set = getFiles(FileStatus.SAFE_TO_DELETE);
    ...
    public synchronized VLSN getFirstVLSN(final Long fileNum) {
        final FileInfo info = fileInfoMap.get(fileNum);
        return (info != null) ? info.firstVlsn : null;
    }
