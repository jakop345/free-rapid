...
public class JoinCursor implements Closeable {
    ...
    public OperationStatus getNext(DatabaseEntry key,
        ...
        secCursors[0].trace(Level.FINEST, "JoinCursor.getNext(key): ",
                            lockMode);
    ...
    public OperationStatus getNext(DatabaseEntry key,
        ...
        secCursors[0].trace(Level.FINEST, "JoinCursor.getNext(key,data): ",
                            lockMode);
        ...
        outerLoop: while (true) {
            ...
            if (!cursorFetchedFirst[0]) {
                ...
                status = secCursor.getCurrentInternal(
                    firstSecKey, candidateKey, lockMode);
                ...
                status = secCursor.retrieveNext(firstSecKey, candidateKey,
                                                lockMode, GetMode.NEXT_DUP);
            }
            ...
            for (int i = 1; i < secCursors.length; i += 1) {
                ...
                if (!cursorFetchedFirst[i]) {
                    status = secCursor.getCurrentInternal(
                        secKey, scratchEntry, lockMode);
                    if (readUncommitted &&
                        status == OperationStatus.KEYEMPTY) {
                        /* Deleted underneath read-uncommitted; skip it. */
                        status = secCursor.retrieveNext(secKey, scratchEntry,
                                                        lockMode,
                                                        GetMode.NEXT_DUP);
                        if (status != OperationStatus.SUCCESS) {
            ...
            if (dataParam != null) {
                final Pair<OperationStatus, RecordVersion> result =
                    secCursors[0].readPrimaryAfterGet(
                        priDb, firstSecKey, candidateKey, dataParam, lockMode,
                        readUncommitted, false /*reverseLockOrder*/);
                status = result.first();
                ...
                /*
                 * Copy RecordVersion to all secondary cursors. This is only
                 * possible if they are true secondary cursors, due to the
                 * rules enforced by CursorImpl for primary/secondary cursors.
                 */
                final RecordVersion version = result.second();
                for (int i = 0; i < secCursors.length; i += 1) {
                    final Cursor c = secCursors[i];
                    if (c.isSecondaryCursor()) {
