...
import cz.vity.freerapid.gui.managers.NiceOrder;
...
public class FileActions extends AbstractBean {
    ...
    public void addNewLinksAction(ActionEvent event) {
        ...
        NiceOrder mixfiles = new NiceOrder();
        ...
        if (!showing) {
            ...
            if (dialog.getModalResult() == NewLinksDialog.RESULT_OK) {
                ...
                if (!files.isEmpty()) {
                    //     if (dialog.randomizeList())  Collections.shuffle(files);
                    final boolean makeReorder = dialog.randomizeList();
                    if (makeReorder) mixfiles.reorder(files);
                    final List<DownloadFile> newfiles = mixfiles.getNotPauzed();
                    ...
                    SwingUtilities.invokeLater(new Runnable() {
                        public void run() {
                            if (notPaused) {
                                if (makeReorder) {
                                    dataManager.addToQueue(newfiles);
                                } else dataManager.addToQueue(files);
                            }
