...
public class FlashXServiceImpl extends AbstractFileShareService {
    private static final String CONFIG_FILE = "plugin_FlashX.xml";
    ...
    public void showOptions() throws Exception {
        super.showOptions();
        if (getPluginContext().getDialogSupport().showOKCancelDialog(new SettingsPanel(this), "FlashX settings")) {
            getPluginContext().getConfigurationStorageSupport().storeConfigToFile(config, CONFIG_FILE);
        }
    }

    SettingsConfig getConfig() throws Exception {
        synchronized (FlashXServiceImpl.class) {
